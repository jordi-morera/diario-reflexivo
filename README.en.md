...
No extra layers: the frontend speaks HTTP to Flask, Flask speaks SQL to SQLite, and it speaks to the Claude API only when it's time to generate a reflection. Easy to reason about, easy to explain in an interview.
...
- **SQLite**: a simple file-based database, no external server or complex migrations required.
...
- **CORS enabled**: allows calls from the frontend during development.
...
- Entries are stored **locally in your own SQLite file** (nothing is uploaded to a server of ours)
- Only the entry content is sent to the Claude API, and only when you ask for a reflection
- Your Anthropic API key is configured locally on your machine (environment variable, never hardcoded)
